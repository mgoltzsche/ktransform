...
func TestBackReferenceHandler(t *testing.T) {
	for _, c := range []struct {
		name               string
		strategy           BackReferenceStrategy
		crossNamespace     bool
		inverseRefResolver func(refs []Object, o Object) []string
	}{
		{"ownerReferences", OwnerReferences(), false, secretsByOwnerRef},
		{"annotationReferences", AnnotationReferences(testAnnotation), true, secretsByAnnotationRef},
		{"annotationOrOwnerReferences", AnnotationOrOwnerReferences(testAnnotation), true, secretsByAnnotationOrOwnerRef},
	} {
		t.Run(c.name, func(strategy BackReferenceStrategy, crossNamespace bool, resolver func(refs []Object, o Object) []string) func(*testing.T) {
			return func(t *testing.T) {
				testBackReferenceHandler(t, strategy, crossNamespace, resolver)
			}
		}(c.strategy, c.crossNamespace, c.inverseRefResolver))
	}
}
...
func testBackReferenceHandler(t *testing.T, strategy BackReferenceStrategy, crossNamespace bool, inverseRefs func(refs []Object, o Object) []string) {
	...
	testee := NewBackReferencesHandler(client, strategy)
...
func secretsByAnnotationRef(secrets []Object, o Object) (r []string) {
	kind := o.GetObjectKind().GroupVersionKind().Kind
	checkKind(kind)
	a := fmt.Sprintf("%s.%s/%s/%s", strings.ToLower(kind), testAnnotation, o.GetNamespace(), o.GetName())
	for _, s := range secrets {
		if s.GetAnnotations()[a] == "true" {
			r = append(r, key(s))
		}
	}
	sort.Strings(r)
...
func secretsByAnnotationOrOwnerRef(secrets []Object, o Object) []string {
	r := append(secretsByAnnotationRef(secrets, o), secretsByOwnerRef(secrets, o)...)
	sort.Strings(r)
	return r
}
